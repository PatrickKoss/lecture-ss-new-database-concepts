...
async fn handle_update_key(stream: &mut tokio::net::TcpStream, request: Request, _tx: Option<&Sender<Request>>) -> Result<()> {
    match request.clone().key_value {
        Some(key_value) => {
            hashmap_log::insert(key_value.key.clone(), key_value.value.clone()).await?;
            todo!("to replicate the request we have to write to the channel");
...
async fn delete_key(stream: &mut tokio::net::TcpStream, key: &str,  _request: Request, _tx: Option<&Sender<Request>>) -> Result<()> {
